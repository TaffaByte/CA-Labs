...
module tb_Task1();
    ...
    // Using the parameterized mux from the refactored code
    mux2 #(32) u_mux2 (
        .d0(pc_next_seq),
        .d1(branch_target),
        .sel(PCSrc),
    ...
    // ==========================================
    initial begin
        // Standard dumpfile and dumpvars for waveform analysis
        $dumpfile("tb_Task1.vcd");
        $dumpvars(0, tb_Task1);

        // 1. Initialize Datapath
        ...
        // 2. Test PC incrementing sequentially (Requirement: PC + 4)
        $display("[%0t] Evaluating sequential PC increments (PCSrc = 0)", $time);
        #20; // 2 clock cycles pass (PC should go: 0 -> 4 -> 8)
        
        // 3. Test I-type Immediate Generation
        // Expected PC before next edge: 8 (it will become 12 at t=35)
        // Mock Instruction: ADDI x1, x2, -15  -> expected imm = -15
        // Binary: 111111110001_00010_000_00001_0010011 = 32'hff110093
        inst = 32'hff110093;
        #10; // Advance cycle. PC becomes 12.
        $display("[%0t] I-Type Test: PC = %0d. Expected Imm = -15, Got = %0d", $time, pc_out, $signed(imm));
        
        // 4. Test S-type Immediate Generation
        // Expected PC before next edge: 12 (it will become 16 at t=45)
        // Mock Instruction: SW x1, 16(x2) -> expected imm = 16
        // Binary: 0000000_00001_00010_010_10000_0100011 = 32'h00112823
        inst = 32'h00112823;
        #10; // Advance cycle. PC becomes 16.
        $display("[%0t] S-Type Test: PC = %0d. Expected Imm = 16, Got = %0d", $time, pc_out, $signed(imm));
        
        // 5. Test B-type Immediate and Branching (Requirement: PCSrc = 1 selection)
        // Expected PC before next edge: 16 (it would become 20 sequentially, but we branch)
        // Mock Instruction: BEQ x1, x2, -8 -> branch offset = -8
        // immGen logic extracts half the offset (-4) so branchAdder computes: 16 + (-4 << 1) = 8
